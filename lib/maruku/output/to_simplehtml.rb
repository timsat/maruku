...
module MaRuKu::Out::SimpleHTML
  ...
  def to_simplehtml(context={})
    d = Nokogiri::XML::DocumentFragment.new(xdoc)
    children_to_simplehtml.each do |e|
      d << e
    end
...
'
  ...
  def to_simplehtml_document(context={})
    doc = to_simplehtml_document_tree
  ...
  def to_simplehtml_document_tree
  ...
  def to_simplehtml_hrule
    create_html_element 'hr'
  end

  def to_simplehtml_linebreak
  ...
  def wrap_as_element(name, attributes_to_copy=[])
    ...
    children_to_simplehtml.inject(m, &:<<)
  end
  ...
  [
   ...
   [%w(em code strong hr span dl dd dt address div p pre caption ul h1 h2 h3 h4 h5 h6), []]
    ...
    [*elements].each do |element|
      HTML4Attributes[element] = attributes
    end
  end
  ...
  def to_simplehtml_ul
    if @attributes[:toc]
      # render toc
      @doc.toc.to_simplehtml
    else
      add_ws wrap_as_element('ul')
    end
  end

  def to_simplehtml_paragraph
    add_ws wrap_as_element('p')
  end

  def to_simplehtml_ol
    add_ws wrap_as_element('ol')
  end

  def to_simplehtml_li
    add_ws wrap_as_element('li')
  end

  def to_simplehtml_quote
    add_ws wrap_as_element('blockquote')
  end

  def to_simplehtml_strong
    wrap_as_element('strong')
  end

  def to_simplehtml_emphasis
    wrap_as_element('em')
  end
    ...
    element =
      if use_syntax && code_lang
        begin
          ...
          to_simplehtml_code_using_pre(source, code_lang)
        rescue => e
  ...
  def to_simplehtml_code_using_pre(source, code_lang=nil)
  ...
  def to_simplehtml_immediate_link
  ...
  def to_simplehtml_email_address
    a = create_html_element('a')
  ...
  def to_simplehtml_footnote_reference
    id = self.footnote_id
  ...
  ## Definition lists ###
  def to_simplehtml_definition_list
    add_ws wrap_as_element('dl')
  end

  def to_simplehtml_definition
    children_to_simplehtml
  end

  def to_simplehtml_definition_term
    add_ws wrap_as_element('dt')
  end

  def to_simplehtml_definition_data
    add_ws wrap_as_element('dd')
  end

  def to_simplehtml_table
    ...
    thead = xelem('thead')
    tr = xelem('tr')
    array_to_simplehtml(head).inject(tr, &:<<)
    thead << tr
    table << thead
    ...
    rows.each do |row|
      tr = xelem('tr')
      array_to_simplehtml(row).each_with_index do |x, i|
  ...
  def to_simplehtml_head_cell
    wrap_as_element('th')
  end

  def to_simplehtml_cell
    if @attributes[:scope]
  ...
  def children_to_simplehtml
    array_to_simplehtml(@children)
  end

  def array_to_simplehtml(array)
    e = []
    array.each do |c|
      if c.kind_of?(String)
        e << xtext(c)
      else
        method = c.kind_of?(MaRuKu::MDElement) ? "to_simplehtml_#{c.node_type}" : "to_simplehtml"
  ...
  def to_simplehtml_ref_definition
    []
  end
